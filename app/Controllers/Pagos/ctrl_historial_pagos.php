<?php 
	...
	class ctrl_historial_pagos extends BaseController {
		...
		public function datatable_buscar_socio() {
			...
			$datosSocios = $this->socios->select("id as id_socio")->select("concat(rut, '-', dv) as rut")->select("rol")->select("concat(nombres, ' ', ape_pat, ' ', ape_mat) as nombre")->select("date_format(fecha_entrada, '%d-%m-%Y') as fecha_entrada")->where("estado", 1)->where("id_apr", $id_apr)->findAll();
