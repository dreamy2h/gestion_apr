<?php 
	...
	class Ctrl_arranques extends BaseController {
		...
		public function llenar_cmb_tipo_documento() {
			$this->validar_sesion();
			$datos_tipo_documento = $this->tipo_documento->select("id")->select("glosa as tipo_documento")->where("estado", 1)->findAll();
		...
		public function guardar_arranque() {
			...
			if ($this->arranques->save($datosArranque)) {
				...
				if ($id_arranque == "") {
					$obtener_id = $this->arranques->select("max(id) as id_arranque")->first();
