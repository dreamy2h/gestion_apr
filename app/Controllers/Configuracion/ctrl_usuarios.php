<?php 
	...
	class Ctrl_usuarios extends BaseController {
		...
		public function llenar_cmb_apr() {
			$this->validar_sesion();
			$datos_apr = $this->apr->select("id")->select("nombre as apr")->findAll();
		...
		public function llenar_cmb_region() {
			$this->validar_sesion();
			$datos_regiones = $this->regiones->select("id")->select("nombre as region")->findAll();
		...
		public function llenar_cmb_provincia($id_region) {
			...
			if ($id_region != "null") {
				$datos_provincias = $this->provincias->select("id")->select("nombre as provincia")->where("id_region", $id_region)->findAll();
			} else {
				$datos_provincias = $this->provincias->select("id")->select("nombre as provincia")->findAll();
			}
		...
		public function llenar_cmb_comuna($id_provincia) {
			...
			if ($id_provincia != "null") {
				$datos_comunas = $this->comunas->select("id")->select("nombre as comuna")->where("id_provincia", $id_provincia)->findAll();
			} else {
				$datos_comunas = $this->comunas->select("id")->select("nombre as comuna")->findAll();
			}
			...
			if ($this->request->getMethod() == "post" && $this->validate($this->reglasUsuario)) {
				...
				if ($this->usuarios->save($datosUsuario)) {
					if ($id_usuario != "") {
						...
						echo OK . "--" . $clave;
						$obtener_id = $this->usuarios->select("max(id) as id_usuario")->first();
		...
		public function guardar_permisos_usuario() {
			...
			if ($opcion == "guardar") {
		    	...
		    	if ($filas > 0) {
	    			...
	    			$bind = [$estado, $usuario_reg, $fecha, $id_usuario, $id_permiso];
