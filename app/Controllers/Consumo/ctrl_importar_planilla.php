				...
				if (!$file->isValid()) {
					...
					if ($file->hasMoved()) {
							...
							if ($id_socio != "") {
								...
								if ($filas > 0) {
									...
									if ($datosArranque != "" and $datosArranque["id_medidor"] != "") {
										$datosMedidor = $this->medidores->select("id_diametro")->where("id", $datosArranque["id_medidor"])->first();
										...
										$datosApr = $this->apr->select("tope_subsidio")->where("id", $this->sesión->id_apr_ses)->first();
						...
						if ($this->string_error != "") {
							$error = array('error' => $this->string_error);
