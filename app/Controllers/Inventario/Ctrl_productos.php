	...
	class Ctrl_productos extends BaseController {
		...
		public function guardar_productos() {
			...
			if ($this->productos->save($datosProductos)) {
				...
				if ($id_producto == "") {
					$obtener_id = $this->productos->select("max(id) as id_producto")->first();
				...
				foreach ($datos_detalles as $key) {
					if ($key["codigo_barra"] != "") { $codigo_barra = $key["codigo_barra"]; } else { $codigo_barra = null; }
					if ($key["id_estado"] != "") { $id_estado = $key["id_estado"]; } else { $id_estado = null; }
					if ($key["id_ubicacion"] != "") { $id_ubicacion = $key["id_ubicacion"]; } else { $id_ubicacion = null; }

					$datosDetalleValida = $this->productos_detalles->select("id")->select("codigo")->select("id_estado")->select("id_ubicacion")->select("id_producto")->where("id", $key["id_detalle"])->first();
					...
					if ($key["id_detalle"] != "") {
						...
						$obtener_id_det = $this->productos_detalles->select("max(id) as id_detalle")->first();
		...
		public function llenar_cmb_estado() {
			$this->validar_sesion();
			$datosEstados = $this->estados_producto->select("id")->select("estado_producto as estado")->where("estado", 1)->where("id_apr", $this->sesión->id_apr_ses)->findAll();
