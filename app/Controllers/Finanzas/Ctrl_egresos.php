<?php 
	...
	class Ctrl_egresos extends BaseController {
		...
		public function llenar_cmb_productos() {
			$datosProductos = $this->productos->select("id")->select("nombre as producto")->select("marca")->select("modelo")->findAll();
		...
		public function guardar_compra() {
			...
			$obtener_id = $this->compras->select("max(id) as id_compra")->first();
